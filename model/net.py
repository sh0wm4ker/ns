...
# ==========================================
# [新增] 深度可分离卷积模块 (Encoder用)
# ==========================================
class DepthwiseSeparableConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, bias=True):
        super(DepthwiseSeparableConv, self).__init__()
        # 1. Depthwise: 分组卷积，groups=in_channels，独立处理每个通道的空间信息
        self.depthwise = nn.Conv2d(
            in_channels, in_channels, kernel_size=kernel_size,
            stride=stride, padding=padding, groups=in_channels, bias=bias
        )
        # 2. Pointwise: 1x1卷积，负责通道间的信息融合
...
# ==========================================
# [新增] 深度可分离转置卷积模块 (Decoder用)
# ==========================================
class DepthwiseSeparableTransposeConv(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, output_padding, bias=True):
        super(DepthwiseSeparableTransposeConv, self).__init__()
        # 逆过程：先 Pointwise 调整通道，再 Depthwise Transpose 恢复空间分辨率
...
class analysisTransformModel(nn.Module):
    def __init__(self, in_dim, num_filters, conv_trainable=True):
        ...
        self.transform = nn.Sequential(
            ...
            # 这里为了最大化轻量化，我们将其替换为深度可分离卷积。
            # 原代码: nn.Conv2d(in_dim, num_filters[0], 5, 2, 0),
            DepthwiseSeparableConv(in_dim, num_filters[0], 5, 2, 0),
            GDN(num_filters[0]),

            nn.ZeroPad2d((1, 2, 1, 2)),
            # 第2层替换
            # 原代码: nn.Conv2d(num_filters[0], num_filters[1], 5, 2, 0),
            DepthwiseSeparableConv(num_filters[0], num_filters[1], 5, 2, 0),
            GDN(num_filters[1]),

            nn.ZeroPad2d((1, 2, 1, 2)),
            # 第3层替换
            # 原代码: nn.Conv2d(num_filters[1], num_filters[2], 5, 2, 0),
            DepthwiseSeparableConv(num_filters[1], num_filters[2], 5, 2, 0),
            GDN(num_filters[2]),

            nn.ZeroPad2d((1, 2, 1, 2)),
            # 第4层替换
            # 原代码: nn.Conv2d(num_filters[2], num_filters[3], 5, 2, 0)
            DepthwiseSeparableConv(num_filters[2], num_filters[3], 5, 2, 0)
        )
...
class synthesisTransformModel(nn.Module):
    def __init__(self, in_dim, num_filters, conv_trainable=True):
        ...
        self.transform = nn.Sequential(
            ...
            # 第1层替换
            # 原代码: nn.ConvTranspose2d(in_dim, num_filters[0], 5, 2, 3, output_padding=1),
            DepthwiseSeparableTransposeConv(in_dim, num_filters[0], 5, 2, 3, output_padding=1),
            IGDN(num_filters[0], inverse=True),

            nn.ZeroPad2d((1, 0, 1, 0)),
            # 第2层替换
            # 原代码: nn.ConvTranspose2d(num_filters[0], num_filters[1], 5, 2, 3, output_padding=1),
            DepthwiseSeparableTransposeConv(num_filters[0], num_filters[1], 5, 2, 3, output_padding=1),
            IGDN(num_filters[1], inverse=True),

            nn.ZeroPad2d((1, 0, 1, 0)),
            # 第3层替换
            # 原代码: nn.ConvTranspose2d(num_filters[1], num_filters[2], 5, 2, 3, output_padding=1),
            DepthwiseSeparableTransposeConv(num_filters[1], num_filters[2], 5, 2, 3, output_padding=1),
            IGDN(num_filters[2], inverse=True),

            nn.ZeroPad2d((1, 0, 1, 0)),
            # 第4层替换
            # 原代码: nn.ConvTranspose2d(num_filters[2], num_filters[3], 5, 2, 3, output_padding=1),
            DepthwiseSeparableTransposeConv(num_filters[2], num_filters[3], 5, 2, 3, output_padding=1),
...
# 以下代码保持不变，直接从原文件复制即可，确保Net类正常运行
# ... (原文件中的 Space2Depth, Depth2Space, h_analysisTransformModel, h_synthesisTransformModel,
#      BlockSample, NeighborSample, GaussianModel, PredictionModel_Context,
#      conv_generator, Syntax_Model, PredictionModel_Syntax, BypassRound, Net 等类)

# 为了方便你直接使用，这里补充完整后续所有必须的类（未修改部分）:
